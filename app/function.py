...
def Search_bytotal(R, T, L, M, S, S_date, E_date):
	con = sqlite3.connect("petsitting.db")
	cursor = con.cursor()
	cursor.execute("SELECT Host, Cost_L, Cost_M, Cost_S, Total, H_name FROM petsitter WHERE Total >= ? AND Large >= ? AND Medium >= ? AND Small >=? AND Start_Date <=? AND End_Date >= ? AND Except_Date NOT BETWEEN ? AND ?", (T, L, M, S, S_date, E_date, S_date, E_date))
	data = cursor.fetchall()
	cursor.execute("SELECT COUNT(Host) FROM petsitter WHERE Total >= ? AND Large >= ? AND Medium >= ? AND Small >=? AND Start_Date <=? AND End_Date >= ? AND Except_Date NOT BETWEEN ? AND ?", (T, L, M, S, S_date, E_date, S_date, E_date))
	cnt = cursor.fetchone()
	if data ==[]:
		return 0
	cursor2 = con.cursor()
...
def Make_db_house_new():
	con = sqlite3.connect("petsitting.db")
	cursor = con.cursor()
	cursor.execute("CREATE TABLE IF NOT EXISTS house(Host text NOT NULL,State text NOT NULL, City text NOT NULL, Street text NOT NULL, Apt text NOT NULL, Citycode text DEFAULT 0 ,Type text DEFAULT 0, Room text DEFAULT 0, PRIMARY KEY(Host), CONSTRAINT fk_house FOREIGN KEY (Host) REFERENCES member(Email))")
	con.commit()
	con.close()
...
def Save_house_address_new(E, H_State, H_City, H_Street, H_Apt,H_Zipcode):
	con = sqlite3.connect("petsitting.db")
	cursor = con.cursor()
	cursor.execute("INSERT INTO house(Host, State, City, Street, Apt, Citycode) VALUES (?, ?, ?,?,?,?)", (E,H_State, H_City, H_Street, H_Apt, H_Zipcode))
	con.commit()
	con.close()
...
def Read_petsitter_info():
    con = sqlite3.connect("global.db")
    cursor = con.cursor()
    cursor.execute("SELECT * FROM petsitter_info")
    data = cursor.fetchall()
    con.commit()
    con.close()
    return data

def delete_petsitter_info():
    con = sqlite3.connect("global.db")
    cursor = con.cursor()
    cursor.execute("DELETE FROM petsitter_info")
    con.commit()
    con.close()
...
def Make_db_user_search():
    con = sqlite3.connect("global.db")
    cursor = con.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS user_search(email text, guests int, adults int, children int, infants text, checkin text, checkout text)")
    con.close()
...
def delete_user_search():
    con = sqlite3.connect("global.db")
    cursor = con.cursor()
    cursor.execute("DELETE FROM user_search")
    con.commit()
    con.close()
...
def Make_db_petsitter_num():
    con = sqlite3.connect("global.db")
    cursor = con.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS petsitter_num(count int)")
    con.close()

def Save_petsitter_num(count):
    con = sqlite3.connect("global.db")
    cursor = con.cursor()
    cursor.execute("INSERT INTO petsitter_num(count) VALUES (?)", (count,))
    con.commit()
    con.close()

def Read_petsitter_num():
    con = sqlite3.connect("global.db")
    cursor = con.cursor()
    cursor.execute("SELECT * FROM petsitter_num")
    data = cursor.fetchall()
    con.commit()
    con.close()
    return data

def delete_petsitter_num():
    con = sqlite3.connect("global.db")
    cursor = con.cursor()
    cursor.execute("DELETE FROM petsitter_num")
    con.commit()
    con.close()
...
def Make_db_charge():
    con = sqlite3.connect("global.db")
    cursor = con.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS charge(total_charge int)")
    con.close()

def Save_charge(total_charge):
    con = sqlite3.connect("global.db")
    cursor = con.cursor()
    cursor.execute("INSERT INTO charge(total_charge ) VALUES (?)", (total_charge ,))
    con.commit()
    con.close()
...
def delete_charge():
    con = sqlite3.connect("global.db")
    cursor = con.cursor()
    cursor.execute("DELETE FROM charge")
    con.commit()
    con.close()

def modify_charge(total_charge):
    con = sqlite3.connect("global.db")
    cursor = con.cursor()
    cursor.execute("UPDATE  charge SET total_charge=?", (total_charge ,))
    con.commit()
    con.close()
